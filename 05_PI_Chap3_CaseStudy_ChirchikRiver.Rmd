...
**Charvak reservoir dam, looking downstream. Source: Tobias Siegfried, hydrosolutions GmbH.**

In this Chapter, two Central Asian river basins are presented and extensively discussed. The purpose is to familiarize the reader with key hydrological processes but also to demonstrate ways of hydro-meteorological time series analyses with R [@R-2013].

## Chirchik River Basin {#CRB}

### Overview {#CRBOverview}
...
### Key Basin Characteristics {#CRBBasinCharacteristics}
...
#### Hydrological Characterization {#CRBHydroCharacteristics}
...
#### Climatological Characterization {#CRB_Climatology}
...
### The Estimation of Discharge Contributions by the Ungauged Kosku Tributary {#CRB_KoskuDischargeContribution}
...
This dataframe contains all available data hydro-meteorological data from the basin. Data from the following gauging and meteorological stations are available:

```{r echo=FALSE, fig.cap="Overview over meteorological stations from which data is available. Please note that the discharge station at Khorog (17050) is located very close to the metoerological station 38954 and hence not listed separately. lat and lon are latitiude and longitude. utm.x and utm.y are station locations in UTM42. masl denotes the approximate station location."}
## MeteoStation locations and elevation
meteoStations <- # just put everything in a regular dataframe
  tibble(StationName=c("Bulunkul","Khorog","Javshangoz","Navobod"),
         StationCode = c(38953, 38954, 38956, 38950),
         lat   = c(37.70416667,37.50361111,37.39083333,37.59416667),
         lon   = c(72.94583333,71.515,72.29583333,71.86555556),
         utm.x = NA,
         utm.y = NA,
...
Most of the data are only available at monthly resolution.

As compared to the Chirchik river basin, we only have one long-term operational gauging station in the Gunt river basin. The Station code is 17050. Its discharge data can be accessed and extracted from the Gunt dataset in the following way
...
When we plot the data, we see that we have a near complete monthly record from 1940 onwards. The data gap in the 1990ies was during the Tajik civil war which was a very difficult time for Tajikistan.
...
Please also note the visible changes in the low flow regime from 2007 onwards.

#### Climatology {#GuntRBClimatology}
