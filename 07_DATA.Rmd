...
```{r ClimateImpactModelChain, echo = FALSE, fig.cap="Illustration of the data preparation and modeling steps required to study climate change impact on Central Asian river catchments."}
...
This Section follows the steps shown in Figure \@ref(fig:ClimateImpactModelChain) and explains in a hands-on manner how to arrive at and process the required geospatial data for later inclusion in the hydrological-hydraulic model RS Minerve Links to detailed descriptions of the individual tasks allow even beginners of QGIS to follow the procedure. The requirements for GIS layers for import in RS Minerve are summarized in Figure \@ref(fig:requiredGISInputFilesData).  
```{r requiredGISInputFilesData,echo = FALSE, fig.cap = 'Required GIS input files for RS Minerve. See corresponding user manual for more information.'}
knitr::include_graphics('_bookdown_files/FIG_RS_MINERVE/required_GIS_Input_Files.png')
```
RS Minerve connects the sub-basins to junctions. River stretches run from upstream junction to downstream junction. The names of the junctions need to be consistent throughout the GIS layers.   
In this chapter we will demonstrate how to derive the GIS layers for the Nauvalisoy river catchment. The Nauvalisoy is a small tributary to the Chirchiq river (see Figure \@ref(fig:ChirchikRiverOverview) in the case studies). It does not include channel routing. 

```{r, eval = "html", label="gis-prep", results='asis', echo=FALSE, purl=FALSE}
cat('
<div class = "exercise" <div class = "exercise" id="ex5.1"> 
<h3 class = "right">Ex5.1</h3> 
<h3 class= "left">Prepare GIS layers</h3> 
<p>Go through the steps in this Chapter to generate yourselves the GIS layers of your sample catchment for import into RS Minerve.</p> 
</div>')
```

### Prerequisites
* QGIS must be installed ([QGIS installation guide](#appendix-sofware-qgis-installation-guide)).    
* Coordinates of a discharge gauge in your sample catchment are available in the *learningFiles* directory.   
* A digital elevation model (DEM) for your sample catchment is available in the *learningFiles* directory. A description of how to obtain a free SRTM DEM is given below.  


### Setting up QGIS {#section-geospatial-data-steps-setting-up}

* Open and save a new QGIS project ([How to for absolute beginners](#section-saving-a-new-qgis-project)).   
* Make sure that the projects projection is in UTM ([How to change the projection of a project](#section-change-project-projection-qgis)).   
* Check if the required plugins (qProf or profile tool) are installed ([How to install and activate a plugin](#section-install-and-activate-plugins-in-QGIS3)).   
   + The optional plugins are: SRTM plugin (requires a login to USGS Earth Explorer [How to register](#section-register-with-earth-explorer)).    
* Note that SAGA GIS and GRASS GIS must be installed and correctly configured in QGIS (which is typically the case for default installations of long-term support versions of QGIS).


### Load DEM {#section-geospatial-data-load-dem}

You will find a DEM raster layer in the ./learningFiles directory of your sample catchment ready to start. Load the DEM into your QGIS project and save the project ([how to](#section-quick-guides-load-dem)). For the hydrological analysis, the DEM needs to be in UTM projection. Look up the projection of your DEM layer and re-project to UTM if necessary ([how to](#section-quick-guides-verify-projection-reproject)).    

The description of how to obtain the DEM is given here: We use the SRTM 1 arc-second global DEM ([Data description](https://www.usgs.gov/centers/eros/science/usgs-eros-archive-digital-elevation-shuttle-radar-topography-mission-srtm-1-arc?qt-science_center_objects=0#qt-science_center_objects)). The DEM tiles are downloaded directly in QGIS3 (SRTM plugin, [How to](#section-srtm-plugin)) or via the USGS Earth Explorer ([How to](#section-earth-explorer-download-srtm)) and merged to a single GIS layer ([How to](#section-merge-srtm-tiles)). In any case you will need to register for an Earth Explorer account [How to register](#section-register-with-earth-explorer).  
Typically, geospatial data from open sources is stored in the standard projection WGS84 (EPSG:4326). The WGS84 is in degrees, minutes and seconds but for hydrological analysis it is more convenient to have spatial layers in the UTM projection. Look up in which UTM Zone your catchmen lies and re-project to that Zone. For the example of the Chirchiq basin, the UTM zone is 42N, i.e. EPSG:32642.    


### Catchment delineation {#section-steps-derive-catchment-boundaries}
For this workshop, shapefiles for the sample watersheds you will model are already available in the shared dropbox folder with the [data for case studies](https://www.dropbox.com/sh/hz5fcot6yvi6tff/AACrjSIQGFsbYEfeI-A05lZra?dl=0) which you should have already downloaded. Load the shape file of your sample catchment into QGIS ([how to add a vector layer](#section-add-vector-layer-to-qgis)).   
In case you'd like to learn how to delineate your own watershed, follow the tutorial below. The steps are also described in this [tutorial on youtube](https://www.youtube.com/watch?v=xwiHQlmEEjw) 

#### Projection of DEM {.unnumbered}
Verify that the DEM is in UTM coordinates and re-project it if necessary ([how to](#section-quick-guides-verify-projection-reproject)).   

#### Fill the sinks of the DEM {#section-why-fill-sinks-dem .unnumbered}
To derive the the catchment area upstream of a discharge gauge, the DEM is traced upstream until elevation values do not increase any more, i.e. until the boundary of the watershed is reached. The SRTM DEM contains the rounded average elevation in each cell of about 25-30m resoluition. Due to the averaging and rounding it may happen, that in a SRTM DEM, an upstream river cell contains lower elevations than the downstream river cell. If the catchment delineation algorithm reaches such a situation, it stops, thinking it has reached the boundary of the watershed. To avoid this, cells that form local depressions should be filled to form a river bed which is continuously increasing in elevation until it reaches the watershed boundary. The SAGA algorithm to do this is called *Fill sinks*.   
Fill the sinks of your DEM and store the processed raster file ([how to in detail](#appendix-quick-guides-fill-sinks)). 

#### Upslope area {.unnumbered}
Determine the area up-slope of your river discharge gauge by running the algorithm *Upslope Area* in QGIS ([detailed instructions](#appendix-quick-guide-upslope-area)). The UTM coordinates of the Nauvalisoy discharge gauge are given in the *riversCentralAsia* package: 
```{r}
dplyr::filter(ChirchikRiverBasin, river=="Nauvalisoy") %>% 
  dplyr::summarize(`Station code` = dplyr::first(code), 
                   `Station name` = dplyr::first(station), 
                   `River name` = dplyr::first(river), 
                   Longitude = dplyr::first(lon_UTM42), 
                   Latitude = dplyr::first(lat_UTM42))
```

The *Upslope Area* function will return a raster file of your catchment area. Convert it to a polygon and save the resulting shape ([how to convert raster to polygon](#appendix-quick-guide-polygonize)).   

Now you have a DEM and a watershed boundary ready. The steps to generate the GIS layers for import in RS Minerve are numerous. We have generated the input files for the sample catchments for you. In case you wish to reproduce the steps you can follow the description below. 


### Run through the process model

A process model is available that generates the raw GIS layers for RS Minerve. Navigate to *Processing* and open the *Graphical Modeller...*. The *Model Designer* window opens. Open the model downloaded from here:  [./model/Basin_Preparation_QGIS.model3]("models/Basin_Preparation_QGIS.model3") (\@ref(fig:qgis-graphical-model-designer)).
 ...
 * A shape file of the basin outline in UTM 42 N. A shape file is provided in the [Case Study Packs folder on Dropbox](https://www.dropbox.com/sh/hz5fcot6yvi6tff/AACrjSIQGFsbYEfeI-A05lZra?dl=0) that you should have already downloaded. [Here](#section-steps-derive-catchment-boundaries) is a description of how to derive the basin outline if you need to do so for a future project.    
 * The extent of the DEM needs to be larger than the boundaries of your catchment. You can use the raw SRTM DEM projected to UTM42N that is provided in the [Case Study Packs folder on Dropbox](https://www.dropbox.com/sh/hz5fcot6yvi6tff/AACrjSIQGFsbYEfeI-A05lZra?dl=0). If you want to download the DEM yourself you find a description of how to do so [here](#section-geospatial-data-load-dem).   

#### Model output {.unnumbered}

Besides the 3 layers required for RS Minerve (compare Figure \@ref(fig:requiredGISInputFilesData)): 

* Basins: BasinElevationBands_Poly_fixed   
* Rivers: RiverChannels  
* Junctions: RiverJunctions  

The model produces a number of additional output files which can be used for verifying the parameterisation which is described in more detail in the next section. 

#### Parameterization of the model

 * BasinShapeBuffer_meters is a buffer around the basin outline to which the DEM is cut. The DEM needs to be slightly larger than the shape line. The default value of 1000 m is sufficient, 500 m work as well.   
 ...
 * The Elevation Bands Table holds the altitude boundaries and class IDs for the elevation bands in the catchment. After a first test run, edit according to your needs. Elevation bands of 200 m intervals are typically chosen to sufficiently refine the elevation dependent snow melt. For your case study you may use an interval for the elevation bands between 500 m for smaller catchments and 1000 m for larger catchments.  
 * River Network Level is a parameters for the resolution of the river network. Values of 7 and 8 have shown good results for smaller and medium sized catchments (Figure \@ref(fig:qgis-river-network-level)).  
...
After reading the instructions above, you are now ready to run the model chain. Press the green triangle in the toolbar of the *Model Designer* window and adjust the parameters in the *Basin_Processing* according to the recommendations above.  
For the example of the small Nauvalisoy river basin, the parameterization is given in Figure \@ref(fig:basin-processing-params-nauvalisoy).  
```{r basin-processing-params-nauvalisoy, echo=FALSE, out.width="50%", fig.align="center", fig.cap="Parameters for the basin processing model chain for the Nauvalisoy catchment."}
knitr::include_graphics("_bookdown_files/FIG_DATA/2fig_BasinProcessingNauvalisoy1.png")
```

#### Resulting layers of the QGIS model chain {.unnumbered}
The Output layers of the QGIS model process are shown in Figure \@ref(fig:qgis-model-output-raw). 
```{r qgis-model-output-raw, echo=FALSE, out.width="80%", fig.align="center", fig.cap="Raw output of the QGIS model chain to derive the GIS layers for import into RS Minerve."}
knitr::include_graphics("_bookdown_files/FIG_DATA/2fig_QGISModelOutput.png")
```

Let's inspect the GIS layers and determine which edits are still necessary. 


### Manually edit the GIS layers for import in RS MINERVE

#### The basin layer {.unnumbered}
Verify that the elevation bands are numbered in the correct order. You can do this for example by opening the attribute table, sorting the IDs in ascending order and click through the layers. The selected feature is highlighted in yellow in the map panel and should move along the as you select the next row (see Figure \@ref(fig:qgis-verify-basin)). 
```{r qgis-verify-basin, echo=FALSE, out.width="80%", fig.align="center", fig.cap="Verify that the elevation bands are in the correcto order."}
knitr::include_graphics("_bookdown_files/FIG_DATA/3fig_checkOrderElevationBands.png")
```
Also, there should only be one row per ID in the attribute table. If the QGIS model process has run through correctly, this should be the case. Each of the sub-basins in the basin layer will be translated to a hydrological model in RS Minerve.    

RS Minerve requires the specification of the downstream junction for each sub-basin (compare Figure \@ref(fig:requiredGISInputFilesData)). Toggle the edit mode by pressing the pen icon in the upper left corner of the attribut table window. Press the *New Field* icon (Figure \@ref(fig:qgis-add-field-attrib-table))
```{r qgis-add-field-attrib-table, echo=FALSE, out.width="50%", fig.align="center", fig.cap="Press *Add New Field* in the toolbar of the attribute table."}
knitr::include_graphics("_bookdown_files/FIG_DATA/3fig_addNewField.png")
```

Fill in the attributes for the new field as shown in Figure 
\@ref(fig:qgis-add-field-attrib-table2):
```{r qgis-add-field-attrib-table2, echo=FALSE, out.width="50%", fig.align="center", fig.cap="Enter the attributes of the new field."}
knitr::include_graphics("_bookdown_files/FIG_DATA/3fig_addNewField2.png")
```

Press OK. A new column called *Junc_down* and containing *NULL* now appears in the attribute table. For small catchments, you can manually enter the name of the downstream junction in the *Junc_down* column while in editing mode. For larger catchments with several tributaries you may want to use the field calculator (**TODO WRITE A GUIDE FOR THIS**). We call the junction at which the contributions from the elevation bands flow together the *Outlet* (Figure \@ref(fig:)). 
```{r qgis-add-field-attrib-table3, echo=FALSE, out.width="50%", fig.align="center", fig.cap="Edit the junction names and save your changes."}
knitr::include_graphics("_bookdown_files/FIG_DATA/3fig_addNewField3.png")
```

Save the edits and un-toggle the edit mode by pressing on the pen icon. You can close the attribute window. If the layer is still temporary, you can store save it to your computer ([how to save a temporary layer](#appendix-quick-guide-save-temp-layer)). 

#### The junctions layer {.unnumbered}
The output of the QGIS model process shows two junctions (see Figure \@ref(fig:qgis-model-output-raw)). We only need to keep the junction at the outlet. Select the RiverJunctions layer in the Layers panel open the attribute table. There select the column with ths spring node, toggle the edit mode and delete the node by pressing on the icon with the red bin. The outlet node is already called *Outlet* so we do not need to change this name, it is already consistent with the basins layer. Save your edits and un-toggle the edit mode. Close the attribute table and save the junctions layer.   

An alternative way to edit the junctions layer, especially if you have larger and more complicate catchments, is via the graphical editor. A detailed description of how to do this is given in the [Appendix](#appendix-section-graphical-edit-junctions)

#### The river layer {.unnumbered}
Although we will not implement river routing in the hydrological model for the Nauvalisoy basin, RS Minerve requires a GIS layer for the river. Edit the RiverChannels layer as follows in the attribute table: Press the *Delete fields* button (Figure \@ref(fig:qgis-delete-fields-button)).
```{r qgis-delete-fields-button, echo=FALSE, out.width="5%", fig.align="center", fig.cap="The delete fields button in the attribute table toolbar."}
knitr::include_graphics("_bookdown_files/FIG_DATA/4fig_deleteColumns.png")
```

In the pop-up window, select all fields but BASIN and LENGTH for deletion and press OK (Figure \@ref(fig:qgis-delete-fields-window)). 
```{r qgis-delete-fields-window, echo=FALSE, out.width="30%", fig.align="center", fig.cap="Select the fields of the attribute table to delete."}
knitr::include_graphics("_bookdown_files/FIG_DATA/4fig_deleteFields.png")
```

Add a field for the downstream junction and enter Outlet you can proceed in the same way as described above for editing the basin layer. Save the river channels layer.   

If you have larger catchment with a more complex structure to edit you may be interested in a detailed description of the graphical editing of GIS layers in the [Appendix](#appendix-section-graphical-edit-junctions). The example shows editing of a junction layer but you may proceed in the same manner for editing the rivers layer. 

The layers are now ready for import into RS Minerve. This will be described in [Chapter 6.4](#section-rsminerve-data-preparation).   
