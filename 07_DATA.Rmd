...
The bias corrected CHELSA precipitation (pr) raster data is available via [`./HydrologicalModeling_CentralAsia/CentralAsiaDomain/CHELSA_V1.2.1/pbcorr_tp`](https://www.dropbox.com/sh/l20s3089ok1orlr/AAALvIdEc43R3zW3enmvq3ZHa?dl=0) . Using these temperature as well bias-corrected precipitation data, we can easily compute and display the monthly norm climatology fields over the Central Asia domain[^data-4]. Here, we just load them and visualize the mean monthly patterns for a consistency check.
...
Using again the example of the Gunt river basin, the following code block shows how to use the function `prepare_RMAWGEN_input_data()` for the preparation of the relevant RMAWGEN input data files.

```{r}
# Load and process RS MINERVE csv database file
filePath <- '/Users/tobiassiegfried/Dropbox (hydrosolutions)/1_HSOL_PROJECTS/PROJECTS/SDC/DKU_WRM_COURSE_CA/CourseMaterials/Data/RSMinerve/AmuDarya/GuntYashikul/GIS/'
fileName <- 'Gunt_Yashikul_1981_2013_db.csv'
era5_gunt <- read.table(paste0(filePath,fileName),sep=',') %>% as_tibble()

# Delete unused catchments in the Gunt RRM
idx2del <- (era5_gunt[1,]=='Gunt_DS_1')|(era5_gunt[1,]=='Gunt_DS_2')|(era5_gunt[1,]=='Gunt_DS_3')
era5_gunt <- era5_gunt %>% dplyr::select(which(!idx2del))

rmawgen_input_files <- prepare_RMAWGEN_input_data(era5_gunt)
rmawgen_input_files %>% list2env(.,envir = .GlobalEnv) # Send list variables to global environment.
```
